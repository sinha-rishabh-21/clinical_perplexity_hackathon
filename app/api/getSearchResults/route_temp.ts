...
const searchResultsSchema: JSONSchema7 = {
  type: "object",
  properties: {
    name: { type: "string" },
    ...
    trial: {
      ...
      properties: {
        ...
        participants_enrolled: { type: "number" },
    ...
    investments: {
      type: "object",
      properties: {
        lead_sponsor: { type: "string" },
        total_money_over_last_10_years_usd: {
          type: "array",
          items: { type: "number" },
        },
      },
      required: ["lead_sponsor", "total_money_over_last_10_years_usd"],
    },
    market_outlook: {
      type: "object",
      properties: {
        estimated_peak_sales_usd: { type: "number" },
        expected_launch_year: { type: "number" },
      },
      required: ["estimated_peak_sales_usd", "expected_launch_year"],
    },
  },
  required: [
    "name",
  ...
  try {
    const response = await sonarClient.chat.completions.create({
      ...
      response_format: {
        type: "json_schema",
        json_schema: {
          name: "search_result", // ✅ required by OpenAI spec
          ...searchResultsSchema, // spread your actual schema here
        },
      },
      messages: [
        {
